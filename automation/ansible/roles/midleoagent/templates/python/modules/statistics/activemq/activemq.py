...
def getStat(thisqm,inpdata):
   try:
      ...
      jar_path = os.path.join(script_dir, "resources", "midleo_activemq.jar")
      usr_value = ""
      pwd_value = ""
      mngm_port = ""
      if "usr" in inpdata:
      ...
      java_arg = json.dumps({"logdir": logdir, "server": thisqm , "mbean": mbeans_combined, "function": "localstat", "usr": usr_value, "pwd": pwd_value, "jmxport": mngm_port})
    ...
    try:
       if len(stat_data)>0:
          for k,item in stat_data.items():
             func = getattr(statarr, "activemq", None)
