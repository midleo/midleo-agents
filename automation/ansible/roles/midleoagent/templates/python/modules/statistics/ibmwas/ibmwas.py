...
def getStat(thisqm,inpdata):
   try:
      ...
      jar_path = os.path.join(script_dir, "resources", "midleo_ibmwas.jar")
      usr_value = ""
      pwd_value = ""
      soap_port = ""
      ssl = "no"
      ...
      if "pwd" in inpdata:
         ...
         del inpdata["pwd"]
      if "soapport" in inpdata:
         soap_port = inpdata["soapport"]
         del inpdata["soapport"]
      if "ssl" in inpdata:
         ssl = inpdata["ssl"]
         del inpdata["ssl"]
      ...
      command = ["java", "-cp", "/midleolibs/libs/*:/midleolibs/vendor/*:" + jar_path, "midleo_ibmwas.ibmwas_main", java_arg]
    ...
    try:
       if len(stat_data)>0:
          for k,item in stat_data.items():
             func = getattr(statarr, "ibmwas", None)
