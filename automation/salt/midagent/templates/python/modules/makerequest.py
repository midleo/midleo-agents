...
def getQStat(webssl,website,webport,qmgr,queue,usr,passwd):
    ...
    try:
        res = requests.get('http'+('s' if webssl=="y" else '')+'://'+website+':'+webport+'/ibmmq/rest/v1/admin/qmgr/'+qmgr+'/queue?name='+queue+'&type=local&attributes=storage.maximumDepth&status=status.currentDepth,status.oldestMessageAge,status.uncommittedMessages,status.lastGet,status.lastPut,status.openInputCount,status.openOutputCount', verify=False, headers=headers, auth=(usr, base64.b64decode(passwd).decode('utf-8').rstrip()))
